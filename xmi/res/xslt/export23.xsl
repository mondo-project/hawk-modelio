...
<xsl:template match="//appliedProfile[not(@xmi:type)][not(contains(./@href,'sysml_Profile'))]">  
	<xsl:copy>
...
<!--
<xsl:template match="//appliedProfile[contains(./@href,'sysml_Profile')]">  
	<xsl:copy>
  	<xsl:attribute name="xmi:type" namespace="http://schema.omg.org/spec/XMI/2.1">uml:Profile</xsl:attribute>
    <xsl:attribute name="href" >http://www.omg.org/spec/SysML/20100301/SysML-profile.uml</xsl:attribute>
	</xsl:copy>
</xsl:template>

<xsl:template match="//eAnnotations[contains(../appliedProfile/@href,'sysml_Profile')]">  
</xsl:template>
!-->
